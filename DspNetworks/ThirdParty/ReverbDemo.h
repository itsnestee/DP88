...
#include "src/ReverbDemo.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct ReverbDemoMetaData {
		SN_NODE_ID("ReverbDemo");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using ReverbDemo = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _ReverbDemo, ReverbDemoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
